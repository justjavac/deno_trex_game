...
function decodeUTF16Base64ToString(encodedText) {
  ...
  for (let i = 0; i < data.length; i += 2) {
    result += String.fromCharCode(
      data.charCodeAt(i) * 256 + data.charCodeAt(i + 1)
    );
  }
  return result;
}
...
function offlineContentAvailable(isShown, suggestions) {
  ...
  for (let index = 0; index < suggestions.length; index++) {
    ...
    ).textContent = decodeUTF16Base64ToString(suggestions[index].title_base64);
    document.getElementById(
      `offline-content-suggestion-attribution-${index}`
    ).textContent = decodeUTF16Base64ToString(
      suggestions[index].attribution_base64
    );
  }
